...
export function ExpensesTab({ data, loading }: ExpensesTabProps) {
  ...
  const { data: hookData, loading: hookLoading } = useModuleData(workspaceId, 'finance', 'expenses')
      ...
      >
        <div className="text-center">
          ...
          <p className="text-muted-foreground">
            {tCommon('loading', { resource: t('tabs.expenses') })}
          </p>
        </div>
      </div>
    )
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <p className="text-muted-foreground">
          {t('descriptions.expenses')}</p>
        <div className="flex items-center gap-2">
          <Button 
            variant="outline" 
            size="sm" 
            className="gap-2"
            aria-label={tCommon('aria.searchButton', { context: t('tabs.expenses') })}
          >
          ...
          <Button 
            size="sm" 
            className="gap-2"
            aria-label={tCommon('aria.createButton', { type: t('tabs.expenses') })}
          >
            <Plus className="h-4 w-4" aria-hidden="true" />
            {tCommon('buttons.create')} {t('tabs.expenses')}
          </Button>
        </div>
      </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base">{t('tabs.expenses')}</CardTitle>
          <CardDescription>{t('descriptions.expenses')}</CardDescription>
        </CardHeader>
        <CardContent>
          {items.length === 0 ? (
            <div className="text-center py-12">
              <div className="text-muted-foreground">
                <p className="text-lg font-semibold mb-2">
                  {tCommon('emptyState.title', { resource: t('tabs.expenses') })}
                </p>
                <p className="text-sm mb-4">{tCommon('emptyState.description')}</p>
                <Button aria-label={tCommon('aria.createButton', { type: t('tabs.expenses') })}>
                  <Plus className="h-4 w-4 mr-2" aria-hidden="true" />
                  {tCommon('emptyState.button', { resource: t('tabs.expenses') })}
                </Button>
              </div>
            </div>
          ) : (
