...
export function PivotView({ data, onItemClick }: PivotViewProps) {
  ...
  const fields = data.length > 0 ? Object.keys(data[0]).filter((key) => 
    !["id", "created_at", "updated_at"].includes(key)
  ) : []

  // Get unique values for row and column fields
  const rowValues = Array.from(new Set(data.map((item) => item[rowField] || "N/A")))
  const columnValues = Array.from(new Set(data.map((item) => item[columnField] || "N/A")))
      ...
      <div className="p-4 border-b bg-muted/30">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <div className="space-y-2">
            ...
            <Select value={aggregateFunc} onValueChange={(v) => setAggregateFunc(v as AggregateFunction)}>
              <SelectTrigger>
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
