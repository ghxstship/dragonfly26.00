...
interface JobsInvoicesTabProps {
  data?: any[]
  loading?: boolean
}

export function JobsInvoicesTab({ data, loading }: JobsInvoicesTabProps) {
  ...
  const { data: hookData, loading: hookLoading } = useModuleData(workspaceId, 'jobs', 'jobs-invoices')
      ...
      >
        <div className="text-center">
          ...
          <p className="text-muted-foreground">
            {tCommon('loading', { resource: t('tabs.invoices') })}
          </p>
        </div>
      </div>
    )
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <p className="text-muted-foreground">
          {t('descriptions.invoices')}</p>
        <div className="flex items-center gap-2">
          <Button 
            variant="outline" 
            size="sm" 
            className="gap-2"
            aria-label={tCommon('aria.searchButton', { context: t('tabs.invoices') })}
          >
          ...
          <Button 
            size="sm" 
            className="gap-2"
            aria-label={tCommon('aria.createButton', { type: t('tabs.invoices') })}
          >
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base">{t('tabs.invoices')}</CardTitle>
          <CardDescription>{t('descriptions.invoices')}</CardDescription>
        </CardHeader>
        <CardContent>
          {items.length === 0 ? (
            <div className="text-center py-12">
              <div className="text-muted-foreground">
                <p className="text-lg font-semibold mb-2">
                  {tCommon('emptyState.title', { resource: t('tabs.invoices') })}
                </p>
                ...
                <Button aria-label={tCommon('aria.createButton', { type: t('tabs.invoices') })}>
                  ...
                  {tCommon('emptyState.button', { resource: t('tabs.invoices') })}
                </Button>
              </div>
            </div>
          ) : (
...
export default JobsInvoicesTab
