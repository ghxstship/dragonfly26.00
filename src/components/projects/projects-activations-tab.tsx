...
export function ActivationsTab({ data, loading }: ActivationsTabProps) {
  const t = useTranslations('production.projects.activations')
  const tCommon = useTranslations('common')
  const params = useParams()
  const workspaceId = params?.workspaceId as string
  
  // Fetch data if not provided
  const { data: hookData, loading: hookLoading, error } = useModuleData(workspaceId, 'projects', 'activations')
    ...
    return (
      <div className="flex items-center justify-center h-full" role="alert" aria-live="assertive">
        <div className="text-center">
          ...
          <p className="text-muted-foreground">Failed to load activations</p>
