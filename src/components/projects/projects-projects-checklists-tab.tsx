...
interface ProjectsChecklistsTabProps {
  data?: Record<string, unknown>[]
  loading?: boolean
}

export function ProjectsChecklistsTab({ data, loading }: ProjectsChecklistsTabProps) {
  const t = useTranslations('production.projects.checklists')
  const tCommon = useTranslations('common')
  const params = useParams()
  const workspaceId = params?.workspaceId as string
  
  // Fetch data if not provided
  const { data: hookData, loading: hookLoading, error } = useModuleData(workspaceId, 'projects', 'projects-checklists')
    ...
    return (
      <div className="flex items-center justify-center h-full" role="alert" aria-live="assertive">
        <div className="text-center">
          ...
          <p className="text-muted-foreground">Failed to load checklists</p>
...
export default ProjectsChecklistsTab
