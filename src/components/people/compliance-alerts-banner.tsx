...
function ComplianceAlertItem({
  ...
  const severityConfig = {
    ...
    warning: {
      ...
      icon: <AlertTriangle aria-hidden="true" className="h-4 w-4 text-yellow-600" />,
      badge: "bg-yellow-100 text-yellow-700 dark:bg-yellow-900 dark:text-yellow-300"
    },
    critical: {
      bg: "bg-red-50 dark:bg-red-950/30",
      border: "border-red-200 dark:border-red-800",
      icon: <AlertCircle aria-hidden="true" className="h-4 w-4 text-red-600" />,
  ...
  return (
    <Alert aria-hidden="true" className={cn(config.bg, config.border, "relative pr-12")}>
      <div className="flex items-start gap-3">
        <div className="mt-0.5">{config.icon}</div>
        <div className="flex-1 min-w-0">
          <AlertDescription aria-hidden="true" className="text-sm">
            {alert.personnel && (
          ...
          <div className="flex flex-wrap flex-col md:flex-row items-center gap-2 mt-2">
            <Badge aria-hidden="true" className={cn("text-xs", config.badge)}>
              {alert.type}
            </Badge>
            {alert.actionUrl && (
              <Button
              ...
              >
                <a href={alert.actionUrl}>
                  View Details
                  <ExternalLink aria-hidden="true" className="h-3 w-3 ml-1" />
                </a>
              </Button>
            )}
          </div>
    ...
    return (
      <div className="flex flex-wrap flex-col md:flex-row items-center gap-2 p-3 bg-green-50 dark:bg-green-950/30 border border-green-200 dark:border-green-800 rounded-lg">
        <div className="h-8 w-8 rounded-full bg-green-100 dark:bg-green-900 flex flex-wrap items-center justify-center">
          <Clock aria-hidden="true" className="h-4 w-4 text-green-600" />
        </div>
    ...
    <div className="flex flex-wrap flex-col md:flex-row items-center gap-2 p-3 bg-yellow-50 dark:bg-yellow-950/30 border border-yellow-200 dark:border-yellow-800 rounded-lg">
      ...
      <div className="flex-1">
        ...
        <div className="flex flex-wrap flex-col md:flex-row items-center gap-2 mt-1">
          {criticalCount > 0 && (
            <Badge aria-hidden="true" className="h-5 text-xs bg-red-100 text-red-700 dark:bg-red-900 dark:text-red-300">
              {criticalCount} Critical
            </Badge>
          )}
          {warningCount > 0 && (
            <Badge aria-hidden="true" className="h-5 text-xs bg-yellow-100 text-yellow-700 dark:bg-yellow-900 dark:text-yellow-300">
