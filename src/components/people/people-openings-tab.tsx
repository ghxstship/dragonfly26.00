...
export function OpeningsTab({ data, loading }: OpeningsTabProps) {
  const t = useTranslations('production.people.openings')
  const tCommon = useTranslations('common')
  const params = useParams()
  const workspaceId = params?.workspaceId as string
  
  // Fetch data if not provided
  const { data: hookData, loading: hookLoading, error } = useModuleData(workspaceId, 'people', 'openings')
  ...
  return (
    <main role="main" aria-label={t('title')}>
      ...
      <Card>
        <CardHeader>
          <CardTitle aria-hidden="true" className="text-base">Openings</CardTitle>
          <CardDescription>View and manage openings</CardDescription>
        </CardHeader>
        <CardContent>
          {items.length === 0 ? (
