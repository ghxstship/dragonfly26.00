...
export function ActivitiesTab({ data, loading }: ActivitiesTabProps) {
  const t = useTranslations('production.events.activities')
  const tCommon = useTranslations('common')
  const params = useParams()
  const workspaceId = params?.workspaceId as string
  
  // Fetch data if not provided
  const { data: hookData, loading: hookLoading, error } = useModuleData(workspaceId, 'events', 'activities')
  ...
  if (error) {
    return (
      <div className="flex items-center justify-center h-full" role="alert" aria-live="assertive">
        <div className="text-center">
          <Activity aria-hidden="true" className="h-8 w-8 text-destructive mx-auto mb-4" />
  ...
  return (
    <main role="main" aria-label={t('title')}>
      ...
      <Card>
        <CardHeader>
          <CardTitle aria-hidden="true" className="text-base">Activities</CardTitle>
          <CardDescription>View and manage activities</CardDescription>
        </CardHeader>
        <CardContent>
          {items.length === 0 ? (
