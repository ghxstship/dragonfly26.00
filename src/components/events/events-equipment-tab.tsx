...
export function EquipmentTab({ data, loading }: EquipmentTabProps) {
  const t = useTranslations('production.events.equipment')
  const tCommon = useTranslations('common')
  const params = useParams()
  const workspaceId = params?.workspaceId as string
  
  // Fetch data if not provided
  const { data: hookData, loading: hookLoading, error } = useModuleData(workspaceId, 'events', 'equipment')
  ...
  return (
    <main role="main" aria-label={t('title')}>
      ...
      <Card>
        <CardHeader>
          <CardTitle aria-hidden="true" className="text-base">Equipment</CardTitle>
