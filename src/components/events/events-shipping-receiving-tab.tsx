...
interface ShippingReceivingTabProps {
  data?: Record<string, unknown>[]
  loading?: boolean
}

export function ShippingReceivingTab({ data, loading }: ShippingReceivingTabProps) {
  const t = useTranslations('production.events.shipping_receiving')
  const tCommon = useTranslations('common')
  const params = useParams()
  const workspaceId = params?.workspaceId as string
  
  // Fetch data if not provided
  const { data: hookData, loading: hookLoading, error } = useModuleData(workspaceId, 'events', 'shipping-receiving')
  ...
  return (
    <main role="main" aria-label={t('title')}>
      ...
      <Card>
        <CardHeader>
          <CardTitle aria-hidden="true" className="text-base">Shipping & Receiving</CardTitle>
          <CardDescription>View and manage shipping & receiving</CardDescription>
        </CardHeader>
        <CardContent>
          {items.length === 0 ? (
            <div className="text-center py-6 md:py-4 md:py-6 lg:py-8 lg:py-12">
              <div className="text-muted-foreground">
                <p className="text-lg font-semibold mb-2">No shipping & receiving found</p>
                <p className="text-sm mb-4">{t('emptyStateMessage')}</p>
                <Button>
                  <Plus aria-hidden="true" className="h-4 w-4 mr-2" />
                  Create Shipping & Receiving
                </Button>
              </div>
            </div>
          ) : (
...
export default ShippingReceivingTab
