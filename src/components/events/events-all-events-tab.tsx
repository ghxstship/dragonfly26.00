...
export function AllEventsTab({ data, loading }: AllEventsTabProps) {
  const t = useTranslations('production.events.all_events')
  const tCommon = useTranslations('common')
  const params = useParams()
  const workspaceId = params?.workspaceId as string
  
  // Fetch data if not provided
  const { data: hookData, loading: hookLoading, error } = useModuleData(workspaceId, 'events', 'all-events')
