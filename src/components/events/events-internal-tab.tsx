...
export function InternalTab({ data, loading }: InternalTabProps) {
  ...
  const { data: hookData, loading: hookLoading, error } = useModuleData(workspaceId, 'events', 'internal')
  ...
  return (
    <main role="main" aria-label={t('title')}>
      ...
      <Card>
        <CardHeader>
          <CardTitle aria-hidden="true" className="text-base">Internal</CardTitle>
          <CardDescription>View and manage internal</CardDescription>
        </CardHeader>
        <CardContent>
          {items.length === 0 ? (
