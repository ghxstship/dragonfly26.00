...
interface InsurancePermitsTabProps {
...
export function InsurancePermitsTab({ data, loading }: InsurancePermitsTabProps) {
  ...
  const { data: hookData, loading: hookLoading, error } = useModuleData(workspaceId, 'files', 'insurance-permits')
  ...
  return (
    <main role="main" aria-label={t('title')}>
      ...
      <Card>
        <CardHeader>
          <CardTitle aria-hidden="true" className="text-base">Insurance & Permits</CardTitle>
          <CardDescription>View and manage insurance & permits</CardDescription>
        </CardHeader>
        <CardContent>
          {items.length === 0 ? (
...
export default InsurancePermitsTab
