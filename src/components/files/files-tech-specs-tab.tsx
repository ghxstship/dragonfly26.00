...
interface TechSpecsTabProps {
  data?: Record<string, unknown>[]
  loading?: boolean
}

export function TechSpecsTab({ data, loading }: TechSpecsTabProps) {
  ...
  const { data: hookData, loading: hookLoading, error } = useModuleData(workspaceId, 'files', 'tech-specs')
  ...
  return (
    <main role="main" aria-label={t('title')}>
      ...
      <Card>
        <CardHeader>
          <CardTitle aria-hidden="true" className="text-base">Tech Specs</CardTitle>
          <CardDescription>View and manage tech specs</CardDescription>
        </CardHeader>
        <CardContent>
          {items.length === 0 ? (
            <div className="text-center py-6 md:py-4 md:py-6 lg:py-8 lg:py-12">
              <div className="text-muted-foreground">
                <p className="text-lg font-semibold mb-2">No tech specs found</p>
