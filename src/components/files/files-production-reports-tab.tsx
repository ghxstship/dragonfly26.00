...
export function ProductionReportsTab({ data, loading }: ProductionReportsTabProps) {
  ...
  const { data: hookData, loading: hookLoading, error } = useModuleData(workspaceId, 'files', 'production-reports')
  ...
  return (
    <main role="main" aria-label={t('title')}>
      ...
      <Card>
        <CardHeader>
          <CardTitle aria-hidden="true" className="text-base">Production Reports</CardTitle>
          <CardDescription>View and manage production reports</CardDescription>
        </CardHeader>
        <CardContent>
          {items.length === 0 ? (
