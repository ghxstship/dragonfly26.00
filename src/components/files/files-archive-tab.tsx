...
export function ArchiveTab({ data, loading }: ArchiveTabProps) {
  const t = useTranslations('production.files.archive')
  const tCommon = useTranslations('common')
  const params = useParams()
  const workspaceId = params?.workspaceId as string
  
  // Fetch data if not provided
  const { data: hookData, loading: hookLoading, error } = useModuleData(workspaceId, 'files', 'archive')
  ...
  return (
    <main role="main" aria-label={t('title')}>
      ...
      <Card>
        <CardHeader>
          <CardTitle aria-hidden="true" className="text-base">Archive</CardTitle>
          <CardDescription>View and manage archive</CardDescription>
        </CardHeader>
        <CardContent>
          {items.length === 0 ? (
            <div className="text-center py-6 md:py-4 md:py-6 lg:py-8 lg:py-12">
              <div className="text-muted-foreground">
                <p className="text-lg font-semibold mb-2">No archive found</p>
