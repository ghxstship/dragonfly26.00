...
export function AccessTab({ data, loading }: AccessTabProps) {
  const t = useTranslations('production.locations.access')
  ...
  const { data: hookData, loading: hookLoading, error } = useModuleData(workspaceId, 'locations', 'access')
  ...
  return (
    <main role="main" aria-label={t('title')}>
      ...
      <Card>
        <CardHeader>
          <CardTitle aria-hidden="true" className="text-base">Access</CardTitle>
          <CardDescription>View and manage access</CardDescription>
        </CardHeader>
        <CardContent>
          {items.length === 0 ? (
            <div className="text-center py-6 md:py-4 md:py-6 lg:py-8 lg:py-12">
              <div className="text-muted-foreground">
                <p className="text-lg font-semibold mb-2">No access found</p>
