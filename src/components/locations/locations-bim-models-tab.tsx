...
interface BimModelsTabProps {
  data?: Record<string, unknown>[]
  loading?: boolean
}

export function BimModelsTab({ data, loading }: BimModelsTabProps) {
  const t = useTranslations('production.locations.bim_models')
  const tCommon = useTranslations('common')
  const params = useParams()
  const workspaceId = params?.workspaceId as string
  
  // Fetch data if not provided
  const { data: hookData, loading: hookLoading, error } = useModuleData(workspaceId, 'locations', 'bim-models')
  ...
  return (
    <main role="main" aria-label={t('title')}>
      ...
      <Card>
        <CardHeader>
          <CardTitle aria-hidden="true" className="text-base">BIM Models</CardTitle>
          <CardDescription>View and manage bim models</CardDescription>
        </CardHeader>
        <CardContent>
          {items.length === 0 ? (
            <div className="text-center py-6 md:py-4 md:py-6 lg:py-8 lg:py-12">
              <div className="text-muted-foreground">
                <p className="text-lg font-semibold mb-2">No bim models found</p>
