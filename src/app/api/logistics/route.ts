...
/**
 * POST - Create new logistics record
 */
export async function POST(request: Request) {
  try {
    ...
    return NextResponse.json(
      { data, message: 'logistics created successfully' },
      { status: 201 }
    )
  } catch (error: any) {
...
/**
 * PUT - Update existing logistics record
 */
export async function PUT(request: Request) {
  try {
    ...
    console.error('PUT logistics error:', error)
    return NextResponse.json(
      { error: error.message || 'Internal server error' },
      { status: 500 }
    )
...
/**
 * DELETE - Remove logistics record
 */
