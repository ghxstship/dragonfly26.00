...
/**
 * GET - Fetch reconciliation records
 */
...
/**
 * POST - Create new reconciliation record
 */
export async function POST(request: Request) {
  try {
    ...
    return NextResponse.json(
      { data, message: 'reconciliation created successfully' },
      { status: 201 }
    )
  } catch (error: any) {
...
/**
 * PUT - Update existing reconciliation record
 */
export async function PUT(request: Request) {
  try {
    ...
    console.error('PUT reconciliation error:', error)
    return NextResponse.json(
      { error: error.message || 'Internal server error' },
      { status: 500 }
    )
...
/**
 * DELETE - Remove reconciliation record
 */
