...
/**
 * POST - Create new forecasts record
 */
export async function POST(request: Request) {
  try {
    ...
    return NextResponse.json(
      { data, message: 'forecasts created successfully' },
      { status: 201 }
    )
  } catch (error: any) {
...
/**
 * PUT - Update existing forecasts record
 */
export async function PUT(request: Request) {
  try {
    ...
    console.error('PUT forecasts error:', error)
    return NextResponse.json(
      { error: error.message || 'Internal server error' },
      { status: 500 }
    )
...
/**
 * DELETE - Remove forecasts record
 */
