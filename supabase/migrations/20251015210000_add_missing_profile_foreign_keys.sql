...
BEGIN
  IF NOT EXISTS (
    ...
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'approval_requests') 
       AND EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'approval_requests' AND column_name = 'resolved_by') THEN
      ALTER TABLE approval_requests
...
BEGIN
  IF NOT EXISTS (
    ...
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'project_costs') THEN
...
DO $$
BEGIN
  RAISE NOTICE 'Migration 20251015210000: Missing profile foreign keys added successfully';
  RAISE NOTICE 'Fixed relationships for: estimates, subcontractor_invoices, compliance_docs, hiring_applications, approval_steps, work_orders';
